...
def get_supabase() -> Client:
    ...
    if _supabase is None:
        ...
        if not url or not key:
            raise ValueError(
                "SUPABASE_URL and SUPABASE_KEY must be set (e.g. in .env) for orchestration."
            )
...
def get_document_collection() -> Collection:
    ...
    if _document_collection is None:
        ...
        if not mongo_url or not database_name or not collection_name:
            raise ValueError(
                "MONGO_URL, DATABASE_NAME, and DOCUMENT_CONTENTS_COLLECTION must be set."
            )
        client = MongoClient(mongo_url)
        _document_collection = client[database_name][collection_name]
...
def get_paper_from_supabase(arxiv_id: str) -> Optional[Dict[str, Any]]:
    try:
        result = get_supabase().table("papers").select("*").eq("arxiv_id", arxiv_id).execute()
    ...
    try:
        ...
        if keywords_to_insert:
            ...
            for i in range(0, len(keywords_to_insert), batch_size):
                ...
                get_supabase().table("keywords").insert(batch).execute()
...
def process_paper_json(json_path: Path) -> bool:
    try:
        ...
        existing = coll.find_one({"paper_id": paper_id})
        if existing:
