...
if (uploadForm) {
  uploadForm.addEventListener("submit", async (e) => {
    ...
    // 6. Kiểm tra token trước khi upload
    ...
    // 7. Gửi bằng XMLHttpRequest (XHR) để có tiến trình upload
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "/uploadfile", true); // Gửi đến endpoint của FastAPI
    
    // Set Authorization header - PHẢI set sau open() nhưng trước send()
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        ...
        const fileName = file ? file.name : (result.filename || "file");
        let message = `Thành công! File "${fileName}" đã được upload.`;
        
        // Hiển thị warning nếu có (khi database không available)
        if (result.warning) {
          message += `\n\n⚠️ ${result.warning}`;
        }
        ...
        if (document.querySelector(".dropzone-content")) {
          document.querySelector(".dropzone-content").style.display = "block";
        }
        ...
        if (courseCustomInput) courseCustomInput.style.display = "none";
      } else {
        // Upload THẤT BẠI
        ...
        if (xhr.status === 401) {
          ...
          // Xóa token cũ và chuyển đến trang login
          ...
          try {
            ...
            errorMessage = `Lỗi từ server (Status: ${xhr.status}). Vui lòng thử lại.`;
          }
        }
