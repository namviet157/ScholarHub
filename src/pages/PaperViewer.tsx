...
const PaperViewer = () => {
  ...
  const { data: paper, isLoading, isError, error } = useScholarPaperDetail(id);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-background">
        <Header />
        <div className="container mx-auto px-4 py-12 text-muted">Đang tải paper…</div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 space-y-4">
          <p className="text-destructive">
            {isError && error instanceof Error ? error.message : "Không tìm thấy paper."}
          </p>
  ...
  const handleAskQuestion = (e: React.FormEvent) => {
    e.preventDefault();
    if (question.trim()) {
      navigate(`/chat?paper=${paper.id}&q=${encodeURIComponent(question.trim())}`);
    }
  };
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      toast.success("Đã copy liên kết vào clipboard.");
    } catch {
      toast.error("Không thể chia sẻ hoặc copy liên kết.");
    }
  };
  ...
  const toggleSave = () => {
    ...
    toggle();
    toast.success(was ? "Đã bỏ lưu." : "Đã thêm vào bookmark.");
  };
